...
const Sidebar = () => {
	return (
		<aside className='h-screen pt-8 relative'>
			...
			<ul className=' mt-8 px-[60px] space-y-5 text-gray-400'>
				<li className=' text-xs'> OTHERS 1 </li>
				<ListItem text="Item 4" ><CameraIcon /></ListItem>
				<ListItem text="Item 5" ><DeleteIcon /></ListItem>
			</ul>

			<ul className=' mt-8 px-[60px] space-y-5 text-gray-400'>
				<li className=' text-xs'> OTHERS 2 </li>
...
const ListItem = ({ children, active = false, text }: { children?: React.ReactElement, active?: boolean, text: string }) => {
	return (
		<>	<li className={`${active && 'text-orange-300'} flex items-center space-x-3`}>
